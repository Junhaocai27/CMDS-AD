...
if "__main__" == __name__:
    ...
    parser.add_argument(
        "--checkpoint",
        type=str,
        default=os.getenv("CMDS_AD_MARIGOLD_CHECKPOINT", str(RELEASE_ROOT / "weights/marigold")),
    ...
    parser.add_argument(
        ...
        choices=["cuda"],
        default="cuda",
        help="Inference device. The release pipeline requires CUDA; use --apple_silicon for MPS.",
    )
    parser.add_argument(
        "--input_rgb_dir",
        type=str,
        required=False,
        default=str(RELEASE_ROOT / "data/derived/marigold_input"),
        help="Path to the input image folder.",
    )
    parser.add_argument(
        "--output_dir", type=str, required=False, default=str(RELEASE_ROOT / "data/derived/marigold_output"), help="Output directory."
    )
    parser.add_argument(
        "--denoise_steps",
        type=int,
    ...
    if apple_silicon:
        if torch.backends.mps.is_available() and torch.backends.mps.is_built():
            ...
            raise RuntimeError("Apple Silicon MPS was requested, but MPS is not available")
    else:
        if not torch.cuda.is_available():
            raise RuntimeError("CUDA is required for Marigold normal estimation, but it is not available")
