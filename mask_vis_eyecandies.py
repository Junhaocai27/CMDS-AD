...
# ==========================================
# 2. 自定义数据集读取类 (适配 Eyecandies)
# ==========================================
class SimpleEyecandiesDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if os.path.exists(xyz_path):
            ...
            xyz_tensor = transforms.functional.to_tensor(xyz_np).float() # [3, H, W]
...
def process_single_class(args, class_name, feature_extractor, device):
    ...
    for split in splits:
        dataset = SimpleEyecandiesDataset(root_dir=args.dataset_path,
...
def run_all_classes(args):
    ...
    if args.class_name.lower() == 'all':
        target_classes = eyecandies_classes
    else:
    ...
    for i, cls in enumerate(target_classes):
        print(f"\n[{i+1}/{len(target_classes)}] Processing class: {cls}")
        try:
...
if __name__ == '__main__':
    ...
    # 默认路径修改为 Eyecandies 格式化的路径
    parser.add_argument('--dataset_path', default='./data/derived/eyecandies_mvtec_format', type=str)
    # 默认保存路径修改为独立文件夹防止覆盖
    parser.add_argument('--save_dir', default='./data/derived/eyecandies_masks_generated', type=str)
